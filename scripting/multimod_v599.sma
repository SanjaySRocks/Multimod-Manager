/*
Credits:
1. Fysiks 	- Idea for loading different set of plugins with same plugins.ini
2. joropito	- Idea for loading different set of plugins independently
3. Galileo 	- voting functions
4. SkyIN	- Testing and ideas
5. Mana		- Testing and ideas
6. zero		- Implementation
*/

/*
To Do:
1. Map Nominations 		(Not interested anymore !!!)
*/
...
new cvar_banLastmod, cvar_changehostname, cvar_playSounds;
...
MakeModMenu(id, bool: custom, const szMenu[], const szHandler[])
{
	...
	new tempLastmod = get_pcvar_num(cvar_banLastmod) ? g_lastmodid : -1;
	
	for(new i = 0 ; i < g_modcount ; i++) 
	{
		if(i == g_currentmodid || (custom && (g_blockedmod[i] || tempLastmod == i)))
			continue;
...
public print_info()
{
	...
	//new pluginsno = ArraySize(g_modplugins[g_currentmodid]);
	...
	{	
		...
		if(i == g_currentmodid)
		{
			charcnt += formatex(szData[charcnt], charsmax(szData) - charcnt, " <Ads - %d> <Cvars - %d> [currentmod]", g_totalads, g_totalcvars);
		}
...
votemod_loadChoices()
{
	...
	log("(votemod_loadChoices) Blocked: %d || Mod count(max): %d || Mod counter: %d", g_totalblocked, g_modcount, mod_counter);
	if(get_pcvar_num(cvar_nominate_enable))
	{
		...
		while(g_choiceCnt < mod_counter && tempNomCnt < g_modcount)
		{
			...
			{
				////log_amx("Rejected - <%s> <%d> <%d>", g_modnames[tempID[g_nomid]], tempID[g_nomvote], tempID[g_nomid]);
				break;
			}
		
			if(tempID[g_nomid] != tempLastmod && tempID[g_nomid] != g_currentmodid)
			{
				copy(g_voteNames[g_choiceCnt], charsmax(g_voteNames[]), g_modnames[tempID[g_nomid]]);
				voteSelID[tempID[g_nomid]] = true;
				////log_amx("Selected - <%s> <%d> <%d>", g_modnames[tempID[g_nomid]], tempID[g_nomvote], tempID[g_nomid]);
				g_choiceCnt++;
			}
			tempNomCnt++;
		}
	...
	while( g_choiceCnt < mod_counter )
	{
		...
		while((g_blockedmod[randID] || g_currentmodid == randID || tempLastmod == randID || voteSelID[randID]) && unsuccessfullcnt < g_modcount )
		{
			unsuccessfullcnt++;
...
votemap_loadChoices()
{
	...
	log("(votemap_loadChoices) Map count(max): %d || Map counter: %d", g_mapcount, map_counter);
	...
	if(get_pcvar_num(cvar_nominate_enable) >= 2)
	{
		...
		for(new k = 0; k < MAX_PLAYERNOMCNT && g_choiceCnt < map_counter; k++)
		{
			...
			{
				playerID = players[i];
				if(g_playernominatedmapcnt[playerID] <= 0)
				{
					nominationFailed++;
					continue;
				}	
				...
				g_playernominatedmapcnt[playerID]--;
			}		
			if(nominationFailed >= playerCnt) break;
		}
		*/
		for(new k = 0; k < MAX_PLAYERNOMCNT && g_choiceCnt < map_counter; k++)
		{
	...
	while( g_choiceCnt < map_counter )
	{
		...
		while((equal(g_currentmap, mapName) || equal(g_lastmap, mapName) || map_isInMenu(mapName)) && unsuccessfullcnt < g_mapcount )
...
MakeMapMenu(id, const modID, const menuID, chooseMode, playerVoteMapCnt = 0)
{
	...
	if(chooseMode == 2)
	{
		//formatex(sel_index, charsmax(sel_index), "%d", mapSize);
	...
	for(new i = 0; i < mapSize; i++)
	{
		...
		switch(chooseMode)
		{
			...
			case 3:
			{
				if(equal(mapline, g_currentmap) || equal(mapline, g_lastmap))
					continue;
