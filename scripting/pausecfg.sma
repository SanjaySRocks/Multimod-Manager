...
public plugin_init()
{
	register_plugin("Pause Plugins", AMXX_VERSION_STR, AUTHOR)
...
{
#if defined MULTIMOD	
	new szTag[10], szMultimodDir[128];
	
	multimod_get_mm_tag(szTag, charsmax(szTag));
	multimod_get_mm_dir(szMultimodDir, charsmax(szMultimodDir));

	format(g_fileToSave, charsmax(g_fileToSave), "%s/%s/%s-pausecfg.ini", g_fileToSave, szMultimodDir, szTag);
#else 
	format(g_fileToSave, charsmax(g_fileToSave), "%s/pausecfg.ini", g_fileToSave);
#endif
